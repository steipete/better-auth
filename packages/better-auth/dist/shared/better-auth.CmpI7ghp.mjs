...
import { l as logger } from './better-auth.D-CMlTs_.mjs';
...
const createAdapter = ({
  ...
  const transformInput = async (data, unsafe_model, action, forceAllowId) => {
    ...
    for (const field in fields) {
      ...
      if (value === void 0 && (!fieldAttributes.defaultValue && !fieldAttributes.transform?.input && !(action === "update" && fieldAttributes.onUpdate) || action === "update" && !fieldAttributes.onUpdate)) {
        continue;
      }
