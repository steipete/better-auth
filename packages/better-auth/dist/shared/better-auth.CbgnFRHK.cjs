...
require('./better-auth.Dg3Pwtlb.cjs');
const session = require('./better-auth.Bco357J8.cjs');
const cookies_index = require('./better-auth.l2-e84v_.cjs');
const schema$1 = require('./better-auth.BIMq4RPW.cjs');
    ...
    endpoints: {
      ...
      userHasPermission: session.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const session$1 = await session.getSessionFromCtx(ctx);
          if (!session$1 && (ctx.request || ctx.headers) && !ctx.body.userId && !ctx.body.role) {
            throw new betterCall.APIError("UNAUTHORIZED");
          }
          const user = session$1?.user || await ctx.context.internalAdapter.findUserById(
            ctx.body.userId
